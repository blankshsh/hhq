...
import { bind_indirect, unbind } from 'api/myCustomer'
...
export default {
  ...
  methods: {
    ...
    getData: function() {
      ...
      Promise.all([bind_indirect(that, $direct), unbind(that, $unbind)]).then(results => {
        console.log(results)
      }).catch((reason) => {
        console.log(reason)
      })
    ...
    scroll01PullingUp() {
      ...
      setTimeout(function() {
        ...
        bind_indirect(that, $direct).then((res) => {
          that.$refs.scroll01.forceUpdate()
